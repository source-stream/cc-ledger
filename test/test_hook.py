...
class HookContract(unittest.TestCase):
    ...
    def test_example_registry_is_valid(self):
        ...
        self.assertIn("globex", reg["groups"])
...
class ArchitectureMap(unittest.TestCase):
    ...
    def test_map_present_marks_current_and_lists_siblings(self):
        repo = make_repo(
            self.tmp / "cdk",
            marker='{"group": "globex", "project": "globex-cdk"}',
        )
        rc, out, err = run_hook(repo)
        self.assertEqual(rc, 0, err)
        ctx = context_of(out)
        self.assertIn("globex-cdk ←you are here", ctx)  # current marked
    ...
    def test_monorepo_current_shows_area_detail(self):
        repo = make_repo(
            self.tmp / "nf",
            marker='{"group": "globex", "project": "globex"}',
        )
        rc, out, err = run_hook(repo)
        self.assertEqual(rc, 0, err)
        ctx = context_of(out)
    ...
    def test_unknown_project_has_no_marker(self):
        repo = make_repo(
            self.tmp / "amb",
            remote="git@github.com:globex/something-unrelated.git",
            marker='{"group": "globex"}',
        )
        rc, out, err = run_hook(repo)
        self.assertEqual(rc, 0, err)
